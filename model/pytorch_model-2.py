...
for epoch in range(num_epochs):
    ...
    for inputs, labels in train_loader:
        ...
        for i in range(inputs.size(0)):
            img = np.transpose(inputs[i].cpu().numpy(), (1, 2, 0))  # Convert to numpy and change channel order
            ...
            img = cv2.cvtColor(equ, cv2.COLOR_GRAY2RGB)  # Convert equalized image back to RGB
            img = np.transpose(img, (2, 0, 1))  # Change channel order back to (C, H, W)
            inputs[i] = torch.from_numpy(img)
    ...
    with torch.no_grad():
        ...
        for inputs, labels in val_loader:
            ...
            for i in range(inputs.size(0)):
                ...
                # Ensure the grayscale image has a single channel and is of type CV_8UC1
                if len(gray.shape) > 2:
                    gray = cv2.cvtColor(gray, cv2.COLOR_BGR2GRAY)
                gray = cv2.convertScaleAbs(gray)  
...
torch.save(model.state_dict(), 'hand_sign_classifier5.pth')
