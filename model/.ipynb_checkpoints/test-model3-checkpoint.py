...
cap = cv2.VideoCapture(0)  # 0 for default camera, you can change to the desired camera index

# Variables for frame extraction and prediction
frame_interval = 30  # Extract a frame every 30 frames (adjust as needed)
frame_count = 0

# Display widget for video stream
...
while True:
    ...
    if frame_count % frame_interval == 0:
        ...
        pil_image_with_prediction = PIL.Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
        display(pil_image_with_prediction)
